resource "aws_vpc" "main" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "private_subnet_cidrs count must match public_subnet_cidrs count."
    }

    precondition {
      condition     = length(var.availability_zones) == 0 || length(var.availability_zones) == length(var.public_subnet_cidrs)
      error_message = "availability_zones must be empty or match public_subnet_cidrs count."
    }
  }
